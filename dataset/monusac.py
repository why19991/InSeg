...
class MoNuSACSegmentation(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # img = Image.open("/data/why/MoNuSAC/train/image_crop/TCGA-IZ-A6M9-01Z-00-DX1_3_1.png").convert('RGB')
        # target = Image.open("/data/why/MoNuSAC/test/mask_crop/TCGA-IZ-A6M9-01Z-00-DX1_3_1.png")
        if self.transform is not None:
            img, target = self.transform(img, target)

        # return img, target, "TCGA-IZ-A6M9-01Z-00-DX1_3_1"
        # return img, target, self.images[index][1][-31:-4]
        return img, target, self.images[index][0][35:]
...
class MoNuSACSegmentationIncremental(data.Dataset):
    def __init__(self,
        ...
        full_monusac = MoNuSACSegmentation(root,image_set=image_Set,is_aug= True,transform=None)
        self.labels = []
        self.labels_old = []
